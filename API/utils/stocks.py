    ...
    if broker_list:
        for broker in broker_list:
            if broker['broker'] == BrokerUser.broker_name:
                raise Exception(f"There is already a broker account saved for {BrokerUser.broker_name}")
    broker_list.append({"broker": user.broker_name.lower(), "broker_username": user.username, "broker_password": user.password})
    try:
        results = mongo.edit_entry(userid=userid, query={"$set": {"brokers": broker_list}})
    except Exception as e:
        print(f"{e}")
        return False
    return results
